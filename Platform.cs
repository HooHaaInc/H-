...
namespace Hi.Android
{
	public class Platform : GameObject
	{
		...
		public Platform(GraphicsDevice graphics, int x, int y){
			worldLocation = new Vector2 (TileMap.TileSize * x, TileMap.TileSize * y);
			defaultLocation = worldLocation;
			using (var stream = TitleContainer.OpenStream("HiContent/Textures/Sprites/ladrilloGris.png")) {
				animations.Add ("plat", new AnimationStrip (
					Texture2D.FromStream(graphics, stream), 64, "plat"));
			}
			animations ["plat"].LoopAnimation = true;
			animations ["plat"].FrameLength = 0.15f;
			animations ["plat"].setSignal (20);

//			using(var stream = TitleContainer.OpenStream("HiContent/Textures/Sprites/"))
